...
function ScratchCard({ onDone: _onDone }: { onDone: () => void }) {
  ...
  return (
    <div style={{ textAlign: "center" }}>
      ...
      {/* Save the Date label — always visible so layout never shifts */}
      <p style={{
        fontFamily:    "var(--font-playfair,'Playfair Display',Georgia,serif)",
      ...
      {/* Fixed-size wrapper — button and date share the same space, no layout shift */}
      <div style={{
        position: "relative",
        ...
        <AnimatePresence mode="wait">
          {!revealed ? (
            ...
            >
              {/* Plain button — no motion captures so mobile scroll is never blocked */}
              <button
                onClick={handleClick}
                style={{
                  width:          "100%",
